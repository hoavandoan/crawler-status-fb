...
class BrowserTypeBase {
    ...
    async launch(options = {}) {
        ...
        const browser = await progress_1.runAbortableTask(progress => this._innerLaunch(progress, options, undefined), timeoutSettings_1.TimeoutSettings.timeout(options), 'browser').catch(e => { throw this._rewriteStartupError(e); });
        return browser;
    }
    async launchPersistentContext(userDataDir, options = {}) {
        options = validateLaunchOptions(options);
        const persistent = options;
        browserContext_1.validateBrowserContextOptions(persistent);
        const browser = await progress_1.runAbortableTask(progress => this._innerLaunch(progress, options, persistent, userDataDir), timeoutSettings_1.TimeoutSettings.timeout(options), 'browser').catch(e => { throw this._rewriteStartupError(e); });
